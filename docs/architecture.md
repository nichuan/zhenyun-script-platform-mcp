...
- 所有 11 个平台持久化/动作工具都执行两阶段协议：第一次只产生绑定工具名与完整参数的短效签名
  计划，不访问写接口；第二次必须携带用户后续明确确认的同一计划。签名过期、复用或参数变化
  均拒绝。
- 确认令牌默认有效 600 秒；这是人工确认计划的 TTL，不是 SSO 密码或平台 access token 的有效期。
- 确认令牌只在当前 MCP 进程内有效；进程重启后未执行计划必须重新生成，避免持久化令牌被重放。
